...
def pistis_workflow_template():
    ...
    def periodic_group():    
        ...
        @task
        def build_conf():
            #conf={"logical_date": , "workflow": "{{ ti.xcom_pull(task_ids='periodic_group.build_periodic_workflow', key='return_value') }}", "access_token": "{{ ti.xcom_pull(task_ids='generate_conf_for_job_dag', key='return_value').job_data.bearer_token }}" },
            context = get_current_context()
            conf = {}
            root_run_id = context["ti"].xcom_pull(task_ids='get_job_from_workflow', key='return_value')['root_dag_run']
            access_token = context["ti"].xcom_pull(task_ids='generate_conf_for_job_dag', key='return_value')['job_data']['bearer_token']
            periodicity = context["params"]["periodicity"]
        ...
        triggering_pistis_periodic_workflow = TriggerDagRunOperator(
            ...
            conf= {"periodicity": "{{ ti.xcom_pull(task_ids='periodic_group.build_conf', key='return_value')['periodicity'] }}", "workflow": json.dumps("{{ ti.xcom_pull(task_ids='periodic_group.build_conf', key='return_value')['workflow'] }}"), "raw_wf": json.dumps("{{ ti.xcom_pull(task_ids='periodic_group.build_conf', key='return_value')['raw_wf'] }}"), "access_token": "{{ ti.xcom_pull(task_ids='generate_conf_for_job_dag', key='return_value')['access_token'] }}", "logical_date": "{{ ti.xcom_pull(task_ids='periodic_group.build_conf', key='return_value')['logical_date'] }}" },
