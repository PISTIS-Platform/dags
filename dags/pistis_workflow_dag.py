...
def pistis_workflow_template():
    ...
    def periodic_group():    
        ...
        triggering_pistis_periodic_workflow = TriggerDagRunOperator(
            ...
            conf= {"periodicity": "{{ ti.xcom_pull(task_ids='periodic_group.build_conf', key='return_value').periodicity }}", "bearer_token": "{{ ti.xcom_pull(task_ids='generate_conf_for_job_dag', key='return_value').access_token }}", "logical_date": "{{ ti.xcom_pull(task_ids='periodic_group.build_conf', key='return_value').logical_date }}" },      
